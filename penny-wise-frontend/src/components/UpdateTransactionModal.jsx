...
export const UpdateTransaction = (props) => {
	...
	const init = () => {
		...
		setTransRepeatFreq(
			trans.transactionRepeatFreq.toLowerCase().substring(0, 1)
		);
		setSelectedTags(trans.transactionTags);
		setNote(trans.note);
		setTransAmount(trans.amount);
		setTransType(trans.transactionType);
		setAvailableTags(
			tags.filter((tag) => {
				return tag.tagType === trans.transactionType;
			})
		);
		setTransDate(trans.transactionDate);
	...
	const handleSubmit = async (event) => {
		...
		const transaction = {
			transactionID: props.trans.transactionID,
	...
	useEffect(() => {
		const getTags = async () => {
			try {
				...
				const selectedIds = [];
				props.trans.transactionTags.forEach((tag) => {
					selectedIds.push(tag.tagID);
				});
				setSelectedTags(props.trans.transactionTags);
				var availTags= await res.data.tags	
				availTags=availTags.filter((tag) => {
					return (
						tag.tagType === props.trans.transactionType &&
						! selectedIds.includes(tag.tagID)
					);
				});
				// availTags = availableTags.filter();
							...
							<Grid container spacing={2}>
								...
								<FormControl
									...
									<OutlinedInput
										...
										defaultValue={props.trans.amount}
									/>
								</FormControl>
								<LocalizationProvider dateAdapter={AdapterDayjs}>
									<DemoContainer
										...
										<DatePicker
											...
											// value={transDate}
											defaultValue={dayjs(
												props.trans.transactionDate.substring(0, 10)
											)}
										/>
									</DemoContainer>
